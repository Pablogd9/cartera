...
def main():
    ...
    prices_eur = {}      # id -> serie EUR [(fecha, precio)]
    last_price = {}      # id -> ultimo precio EUR
    fundamentals = {}    # id -> {pe, ytd, dy}

    for a in assets:
        series, cur = yahoo_history(a["symbol"], rng)
        if not series:
            # fallback: usa el ultimo precio de compra para no dejar valor en 0
            buys = [t for t in txns if t["asset"] == a["id"]]
            if buys:
                lp = buys[-1]["price"]
                last_price[a["id"]] = lp
                WARN.append(f"{a['name']}: sin datos de mercado; uso ultimo precio de compra ({lp}).")
            continue
        if cur and cur != "EUR":
    ...
    weights = {h["id"]: (h["value"] / total_val if total_val else 0) for h in
